...
Partial Class AssignRoleToUser
    Inherits PageBase

    Private UserID As Integer
    Private RoleBLL As New User_RoleBLL
    Private UserBLL As New UserInfoBLL
    Private UserRoleBLL As New User_UserRoleBLL

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            If Page.Request.QueryString("id") Is Nothing Or Not Integer.TryParse(Page.Request.QueryString("id"), UserID) Then
                Throw New NotSupportedException("Invalid Operation")
            End If

            ViewState("UserID") = Page.Request.QueryString("id")
            SetForm()
        Else
    ...
    Private Sub SetForm()
        ...
        Dim thisUser As User_UserInfo = UserBLL.GetUserInfoById(UserID)

        lblCode.Text = thisUser.Code
        lblCName.Text = thisUser.C_Name
        lblEName.Text = thisUser.E_Name
        ...
        If thisUser.RoleIds Is Nothing Then
            thisUser.RoleIds = New List(Of Integer)
        End If

        Dim varUnSelected = From row In dtRole.AsEnumerable _
                  Where loginUser.RoleIds.Contains(Integer.Parse(row("ID"))) _
                        AndAlso Not thisUser.RoleIds.Contains(Integer.Parse(row("ID"))) _
        ...
        Dim varSelected = From row In dtRole.AsEnumerable _
                          Where loginUser.RoleIds.Contains(Integer.Parse(row("ID"))) _
                                AndAlso thisUser.RoleIds.Contains(Integer.Parse(row("ID"))) _
                          Order By row("RoleName").ToString() _
                          Select row
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If roleIDs IsNot Nothing Then
            ...
            Next
        End If
        Dim operatedResult As Boolean = UserRoleBLL.AssignRolesToUser(UserID, roleIDList)
