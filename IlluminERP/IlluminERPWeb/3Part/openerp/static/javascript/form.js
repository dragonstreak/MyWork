...
function openRecord(id, src, target, readonly){
    ...
    var prefix = src && src != '_terp_list' ? src + '../default.htm' : '';
    ...
    var error = jQuery("<table>",{'width': '100%', 'height': '100%'}
                    ...
                    jQuery("<tr>").append(
                        jQuery("<td>", {'css': 'padding: 4px 2px;'}).append(
                            jQuery("<img>", {'src': 'static/images/warning.png'})
                        ),
...
function onChange(caller){
    ...
    var post_url = callback ? 'form/on_change' : 'form/change_default_get';
...
function getName(name, relation){
    ...
    if (value_field.value) {
        var req = openobject.http.getJSON('search/get_name', {
            model: relation,
...
function eval_domain_context_request(options){
    ...
    var parent_context = openobject.dom.get(prefix.concat('_terp_context').join('../default.htm'));
...
function removeAttachment() {
    ...
    jQuery.ajax({
        url: 'attachment/remove/default.htm',
        type: 'POST',
        data: {
...
function add_shortcut_to_bar(id){
    jQuery.getJSON('shortcuts/by_resource', function(data){
        if (data[id]) {
